...
// Ініціалізація захищена: якщо змінні криві чи відсутні — клієнт null,
// сайт працює на демо-даних замість падіння білда.
function initClient(): SupabaseClient | null {
  if (!url || !anon) return null;
  if (!/^https:\/\/.+\.supabase\.co\/?$/.test(url)) return null;
  try {
...
export async function getFilterOptions(type: CategoryType): Promise<FilterOptions> {
  let pool: Product[] = DEMO_PRODUCTS.filter((p) => p.category_type === type);
  if (supabase) {
    try {
      const { data, error } = await supabase
        .from('products')
        .select('brand, price')
        .eq('category_type', type);
      if (!error && data) pool = data as Product[];
    } catch {
...
export async function getAllByType(type: CategoryType): Promise<Product[]> {
  const demo = DEMO_PRODUCTS.filter((p) => p.category_type === type);
  if (!supabase) return demo;
  try {
    const { data, error } = await supabase
      .from('products')
